# Как указать ссылку на карту

<table><tr><td><b>Время чтения:</b> 3 мин.</td><td><b>Обновлено:</b> 14.09.2026</td></tr></table>

<sub>Источник: https://www.5systems.ru/help/kak-ukazat-ssylku-na-kartu</sub>

## Содержание

- [1. Настройка значимого события](#1-настройка-значимого-события)
- [2. Настройка координат](#2-настройка-координат)

В программе предусмотрена возможность отправлять клиенту ссылку на местоположение автосервиса:

![Ссылка на карту - Сообщение](attachments/01-ssylka-na-kartu-soobschenie.png)

Для этого необходимо сделать следующие настройки.

## 1. Настройка значимого события

Необходимо включить значимое событие “Запись на ремонт / Отправить маршрут”. Расположение в справочнике “Значимые события” – *“Прочие события → Чат-бот WhatsApp Business API → Запись на ремонт → Запись на ремонт / Отправить маршрут”:*

![Ссылка на карту - Значимое событие ](attachments/02-ssylka-na-kartu-znachimoe-sobytie.png)

Чтобы включить значимое событие, нужно в карточке события на вкладке “Действия” поставить галочку “Активность”:

![Ссылка на карту - Значимое событие - Активность](attachments/03-ssylka-na-kartu-znachimoe-sobytie-aktivnost.png)

В настройке действия “Отправить маршрут проезда” должна быть выбрана интеграция *WhatsApp Business API через 360dialog*, действие – *“Местоположение”:*

![Ссылка на карту - Действие](attachments/04-ssylka-na-kartu-deystvie.png)

## 2. Настройка координат

Координаты местоположения автосервиса задаются в карточке подразделения компании:

![Ссылка на карту - Координаты ](attachments/05-ssylka-na-kartu-koordinaty.png)

Также должно быть включено значимое событие “Уведомление о записи на ремонт” (см. статью [Настройка шаблонов WhatsApp Business API](../../../articles/5S%20AUTO/Сообщения/WhatsApp/Настройка%20шаблонов%20WhatsApp%20Business%20API/nastroyka-shablonov-whatsapp-business-api.md#marshrut)), и в шаблоне сообщения *“Запись на ремонт/Вы записаны на ремонт”* настроена кнопка *“Маршрут проезда”:*

![Ссылка на карту - Кнопка в шаблоне](attachments/06-ssylka-na-kartu-knopka-v-shablone.png)

---

**Теги:** автоматические сообщения, whatsapp, маршрут проезда, значимые события, координаты

