...
<table><tr><td><b>Время чтения:</b> 8 мин.</td><td><b>Обновлено:</b> 25.09.2026</td></tr></table>

> ![Повышенный уровень сложности](attachments/00-uroven-slozhnosti.png) **Статья для опытных пользователей!**

> *Актуально начиная с версии релиза 5S AUTO **2.8.4.23**.*
...
*Примечание:* если этого пункта нет на рабочем столе, его можно добавить вручную — см. статью «[Настройка рабочего стола](../Настройка%20рабочего%20стола/nastroyka-rabochego-stola.md#настройки-разделов-и-пунктов-рабочего-стола)».
...
![Параметры удаления данных со значениями по умолчанию](attachments/03-Удаление_старых_данных_03-Параметры_по_умолчанию-1.png)
...
> *Комментарий: почему не в часах/минутах?*
...
![Варианты заполнения параметров](attachments/04-Удаление_старых_данных_05-Заполнение-1.png)
...
*Примечание:* В данном примере больше всего места занимают заказ-наряды, но их не чистим — удаление заказ-нарядов не предусмотрено, их важно хранить в программе.
...
- **Журнал обмена** — как правило, самый большой регистр. Он содержит звонки, сообщения, микросервисы и т.д. Его требуется сохранять для себя не менее чем за 5 дней.
- **Журнал событий объектов** — содержимое вкладки «[История](../../CRM/Работа%20со%20сделкой/rabota-so-sdelkoy.md#3-вкладка-история)» в Сделке.
- **Версии объектов** — заполняется при использовании механизма [версионирования](../Версионирование/versionirovanie.md).
- **Отложенные задачи** — обмен с [Calltouch](../../Аналитика/Настройка%20интеграции%20с%20Calltouch/nastroyka-integracii-s-calltouch.md) и [Контакт-Центром](../../Сообщения/Контакт-центр%205S%20Chat/kontakt-centr-5s-chat.md).
...
- **атИнформация** — регистр с информацией об [аналогах](../../Проценка%20запчастей%20и%20работ/Поиск%20цен/poisk-cen.md#область-поиска-и-отображения-результатов-поиска) из каталога Лаксимо. Можно оставлять 1 день.
...
![Рекомендуемые значения для документов](attachments/08-Удаление_старых_данных_08-Параметры_Документы-1.png)

- **Электронное письмо** и **Событие (Пять систем)** — занимают обычно больше всего места среди документов.
- **Событие** — практически не используются, можно оставить за 1–3 месяца.
- **РН Продажи** и **РН Выработка сотрудников** — после удаления данные не будут отражаться в отчетах, поэтому лучше не удалять, но если занимают много места — можно почистить.
...
![Значения по умолчанию для процессов](attachments/09-Удаление_старых_данных_09-Параметры_Процессы-1.png)

- **Задачи** — рекомендуется оставить за 365 дней.
- **Сделки** и **Лиды** — удалять не рекомендуется, по ним должна оставаться статистика.

Остальное можно удалять.
...
![Настройка расписания регламентного задания](attachments/10-Удаление_старых_данных_10-Расписание-1.png)
