...
<table><tr><td><b>Время чтения:</b> 36 мин.</td><td><b>Обновлено:</b> 24.09.2026</td></tr></table>
...
7. [Прочие настройки](#прочие-настройки)
   - [Интеграция с Roistat](#интеграция-с-roistat)
   - [Гарантийная политика](#гарантийная-политика)
   - [Контроль заполнения реквизитов бизнес-процессов](#контроль-заполнения-реквизитов-бизнес-процессов)
   - [Качество обслуживания](#качество-обслуживания)
   - [Поиск сделок и документов](#поиск-сделок-и-документов)
   - [Обновить нумерацию объектов](#обновить-нумерацию-объектов)
...
Описание параметров см. ниже: [*Параметры настройки*](#параметры-настройки).
...
На вкладке **«Прочие настройки»** собраны параметры, которые не относятся к другим вкладкам.

### Интеграция с Roistat

**Доля себестоимости от суммы автоработы, %** – фиксированный процент себестоимости работ, который передается в Roistat. Подробнее – в статье «[Настройка интеграции с системой аналитики Roistat](../../Аналитика/Настройка%20интеграции%20с%20системой%20аналитики%20Roistat/nastroyka-integracii-s-sistemoy-analitiki-roistat.md)».

### Гарантийная политика

**Использовать расширенный функционал гарантийной политики** – включает расширенную гарантийную политику. Подробнее – в статье «[Гарантийная политика](../../Автосервис/Гарантийная%20политика/garantiynaya-politika.md)».

### Контроль заполнения реквизитов бизнес-процессов

В таблице отмечаются реквизиты, обязательные для заполнения в бизнес-процессах, – например, «Контактный телефон». Если отмеченный реквизит не заполнен, бизнес-процесс не записывается. Контроль начинает действовать с даты, указанной в поле **«Дата начала»**.

### Качество обслуживания

Флажки **«Создание документа Жалоба по опросам»**, **«Создание документа Нарушение по документу Жалоба»** и **«Создание произвольной задачи по документу Нарушение»** включают автоматическое создание жалоб по результатам опросов клиентов, нарушений по жалобам и задач по нарушениям. Подробнее – в статьях «[Обработка жалоб от клиентов](../../Управление%20качеством%20обслуживания/Обработка%20жалоб%20от%20клиентов/obrabotka-zhalob-ot-klientov.md)» и «[Преднастроенные значимые события](../../Сообщения/Преднастроенные%20значимые%20события/prednastroennye-znachimye-sobytiya.md)».

### Поиск сделок и документов

> *Параметры «Глубина поиска отказных сделок» и «Глубина поиска успешно завершенных сделок» доступны начиная с релиза 5S AUTO **2.8.5**.*

В группе **«Поиск сделок и документов»** на вкладке **«Прочие настройки»** задается, в пределах какого периода программа ищет сделки клиента при новом обращении:

![Мастер настройки – группа «Поиск сделок и документов»](attachments/27-poisk-sdelok-i-dokumentov.png)

- **Глубина поиска сделок в работе, дн.** – если у клиента есть открытая сделка и с даты ее создания прошло не больше этого периода, обращение попадает в нее; иначе создается новая сделка.
- **Глубина поиска отказных сделок, дн.** – если клиент снова обратился в пределах этого периода от даты отказа, отказная сделка снова становится рабочей. По умолчанию – 2 дня.
- **Глубина поиска успешно завершенных сделок, дн.** – если клиент обратился в пределах этого периода от даты закрытия сделки, новая сделка не создается. По умолчанию – 1 день.

Подробнее о том, как программа выбирает сделку для нового обращения, – в статье «[Работа со сделкой → Когда создается новая сделка](../../CRM/Работа%20со%20сделкой/rabota-so-sdelkoy.md#когда-создается-новая-сделка)».

Другие параметры группы:

- **Создавать сделку при записи корзины** – если для клиента создана корзина, а сделки по нему нет, программа создает сделку автоматически.
- **Глубина поиска заказ-нарядов и заказов покупателей, дн.** – период, за который программа ищет заказ-наряды и заказы покупателей, чтобы определить клиента, когда известен только автомобиль.
- **Период выдачи документов для мобильного приложения, дн.** – период, за который документы передаются в мобильное приложение. Сейчас приоритет у значения из настроек интеграции с мобильным приложением.
- **Контролировать соответствие обращений по реквизитам** – реквизиты, по которым программа ищет существующие сделки клиента при новом обращении, в том числе при определении дублирующих сделок: «Контрагент», «Автомобиль», «ПодразделениеКомпании». Сделка считается найденной, если в ней совпадают все отмеченные реквизиты. Подробнее – в статье «[Объединение сделок → Описание](../../CRM/Объединение%20сделок/obedinenie-sdelok.md#описание)».

### Обновить нумерацию объектов

Кнопка **«Обновить нумерацию объектов»** очищает кэш нумерации всех объектов программы. В этом кэше резервируются номера для объектов, которые еще не записаны. Используйте кнопку, если кэш нумерации сбился.
