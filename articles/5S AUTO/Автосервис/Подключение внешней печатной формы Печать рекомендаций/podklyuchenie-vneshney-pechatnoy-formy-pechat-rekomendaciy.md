...
<table><tr><td><b>Время чтения:</b> 6 мин.</td><td><b>Обновлено:</b> 26.04.2024</td></tr></table>
...
Для подключения внешней формы печати рекомендаций в справочнике **«Внешние печатные формы»** в папке «Внешние печатные формы» создается элемент «Печать рекомендаций», задается наименование, выбирается вид обработки «Печатная форма». Сама форма загружается нажатием на кнопку «Загрузить» ![Кнопка Загрузить](attachments/btn-zagruzit.jpg) *(файл для загрузки: `5с Печатная форма ПечатьРекомендаций.zip` — приложен к исходной статье в разделе «Файлы для скачивания»):*
