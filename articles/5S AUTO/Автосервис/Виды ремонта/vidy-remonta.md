...
**1)** признак *«Ремонт агрегатов»* используется при работе с механизмом [Учет агрегатного ремонта](../Ремонт%20агрегатов/Учет%20агрегатного%20ремонта/uchet-agregatnogo-remonta.md);
...
**5.** При отметке **«Отправлять уведомления»** появляется вкладка «Настройка уведомлений» (см. ниже: раздел «[*Вкладки п.4*](#вкладки)»):
...
**8.** Флаг **«Контролировать детали применяемости»** должен быть установлен для тех видов ремонта, по которым предусмотрена работа по *автозаполнению применяемости из заказ-наряда* — подробнее см. [статью](../../Склад%20и%20запчасти/Применяемость%20запчастей/Автозаполнение%20применяемости%20из%20заказ-наряда/avtozapolnenie-primenyaemosti-iz-zakaz-naryada.md) *(доступно начиная с релиза 2.6.1.1.).* При установке флага будет отображаться колонка «Деталь применяемости» в заказ-наряде в таблицах «Работы» и «Товары».
...
2. Во вкладке **«Работы по умолчанию»** можно добавить дополнительные работы для автозаполнения [Записи на ремонт](../../Запись%20на%20ремонт/) при выборе этого вида ремонта:
...
4. ***«Настройка уведомлений»:*** *«Кому отправляем» — Плательщик / Заказчик / Хозяин.*
   ...
   *Примечание:* Для работы механизма должно быть настроено условие *отправки уведомлений по значимым событиям* (см. [Мастер настройки системы → Настройка отправки сообщений клиентам](../../Администрирование%20и%20настройка%20системы/Мастер%20настройки%20системы/master-nastroyki-sistemy.md)): *Отправлять уведомления (Вид ремонта) = «Да»*.
...
и в [Заказ-наряде](../Работа%20с%20Заказ-нарядами/Заказ-наряды/zakaz-naryady.md):

![Виды ремонта - Использование в Заказ-наряде](attachments/12-ispolzovanie-v-zakaz-naryade.png)

По умолчанию подтягивается вид «Текущий ремонт». В случае необходимости можно настроить пользователю другой вид ремонта по умолчанию через [Установку прав и настроек](../../Пользователи%20и%20права/Установка%20прав%20и%20настроек%20пользователя/ustanovka-prav-i-nastroek-polzovatelya.md) — *Установка прав и настроек → Документы → Общие параметры документов → Ввод новых документов → Вид ремонта по умолчанию:*
