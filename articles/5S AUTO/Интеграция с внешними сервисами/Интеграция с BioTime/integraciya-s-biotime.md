...
<table><tr><td><b>Время чтения:</b> 21 мин.</td><td><b>Обновлено:</b> 16.09.2024</td></tr></table>
...
1. *Заполняется фактическое рабочее время* в табеле учета (см. [*Табель учета рабочего времени → Заполнение из BioTime*](../../Персонал%20и%20мотивация/Табель%20учета%20рабочего%20времени/tabel-ucheta-rabochego-vremeni.md#заполнение-из-biotime));
2. *Создаются нарушения* за опоздания или отсутствие отметки в BioTime (см. [*Выявление и разбор нарушений*](../../Персонал%20и%20мотивация/Выявление%20и%20разбор%20нарушений/vyyavlenie-i-razbor-narusheniy.md)).
...
- *Логин[\*](#snoska-1)* – логин для подключения к серверу BIOTIME 8.
- *Пароль[\*](#snoska-1)* – пароль для подключения к серверу BIOTIME 8.
- *Идентификатор (ClientId)[\*\*](#snoska-2)* – идентификатор ключа доступа приложения для получения токена BIOTIME 8.
- *Секретный ключ (ClientSecret)[\*\*](#snoska-2)* – ключ доступа приложения для получения токена BIOTIME 8.
- *Идентификатор интеграции* – используется, если необходимо создать несколько интеграций с одним и тем же видом интеграции.

<a id="snoska-1"></a>*\*Логин* и *Пароль* – это логин и пароль от входа в личный кабинет на сайте BioTime.

<a id="snoska-2"></a>*\*\*Идентификатор (ClientId)* и *Секретный ключ* необходимо скопировать в личном кабинете на сайте BioTime: *Управление → Настройки системы →* вкладка *Приложения*. Если ключи доступа не созданы или если уже существует интеграция BioTime с другим приложением помимо 1С, следует создать новый ключ – нажать кнопку “Добавить ключ доступа для приложения”:
...
Предусмотрена возможность[\*](#snoska-3) удаления сотрудника из BioTime: для этого можно кликнуть на него правой клавишей мыши и нажать в выпадающем меню кнопку *“Удалить сотрудника из BioTime”:*

![BioTime - Удаление](attachments/20-biotime-udalenie.png)

<a id="snoska-3"></a>*\*Удалять сотрудников таким способом не рекомендуется!*
...
Интеграция с системой BioTime используется для заполнения *фактического рабочего времени* в *табеле учета* (см. *[Табель учета рабочего времени → Заполнение из BioTime](../../Персонал%20и%20мотивация/Табель%20учета%20рабочего%20времени/tabel-ucheta-rabochego-vremeni.md#заполнение-из-biotime)).*
