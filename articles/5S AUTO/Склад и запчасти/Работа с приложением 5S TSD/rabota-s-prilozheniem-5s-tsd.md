...
***5S TSD*** – это вспомогательное приложение для пользователей программы 5S AUTO, предназначенное для автоматизации складских операций. Приложение позволяет работать со штрихкодами и [маркировкой "Честный Знак"](../../Маркировка/Маркировка%20товаров/markirovka-tovarov.md), минимизируя ручной ввод данных и снижая риск ошибок учета.
...
**1)** на любое специализированное устройство сбора данных (ТСД) со встроенным сканером;<br>
...
Приложение запускается через браузер, требуется авторизация под учетной записью [5S Cloud](../../Администрирование%20и%20настройка%20системы/Создание%20и%20редактирование%20пользователей%205S%20Cloud/sozdanie-i-redaktirovanie-polzovateley-5s-cloud.md). Есть возможность установить приложение на устройство и использовать ярлык для упрощенного запуска (см. [ниже](#первый-запуск-и-установка)).

*Для работы с функционалом предварительно должны быть сделаны следующие настройки:*

**1)** подключен сканер ш/к либо ТСД;<br>
**2)** настроена интеграция с [API 5S AUTO](../../Администрирование%20и%20настройка%20системы/API%205S%20AUTO/api-5s-auto.md);<br>
**3)** настроен пользователь [5S Cloud](../../Администрирование%20и%20настройка%20системы/Создание%20и%20редактирование%20пользователей%205S%20Cloud/sozdanie-i-redaktirovanie-polzovateley-5s-cloud.md);<br>
**4)** настроен доступ к складам у пользователя 1С;<br>
**5)** [интеграция с "Честным Знаком"](../../Маркировка/Настройка%20интеграции%20с%20Честным%20знаком/nastroyka-integracii-s-chestnym-znakom.md) для работы с маркированными товарами.

*Также рекомендуем для изучения статьи:*

- [Интерфейс для ТСД](../Интерфейс%20для%20ТСД/interfeys-dlya-tsd.md);
- [Подключение сканера штрихкодов](../../Торговое%20оборудование/Подключение%20сканера%20штрихкодов/podklyuchenie-skanera-shtrikhkodov.md);
...
- статьи в разделе "[Маркировка](../../Маркировка/)" о работе с маркированными товарами.

## Первый запуск и установка

**Шаг 1. Авторизация**

Для первого запуска приложения следует открыть в браузере на устройстве страницу [https://tsd.5systems.ru/](https://tsd.5systems.ru/). Необходимо ввести учетные данные [5S Cloud](../../Администрирование%20и%20настройка%20системы/Создание%20и%20редактирование%20пользователей%205S%20Cloud/sozdanie-i-redaktirovanie-polzovateley-5s-cloud.md):
...
**1.** "Поступление товаров"<br>
**2.** "Отгрузка"<br>
**3.** "Инвентаризация"<br>
**4.** "Перемещение товаров"<br>
**5.** "Остатки"
...
**1.** *Сверка поступления с данными поставщика*<br>
В программу загружается документ поступления от поставщика через ЭДО либо с помощью механизма [загрузки накладных](../Загрузка%20накладных%20поставщиков/zagruzka-nakladnykh-postavschikov.md) через электронную почту.
...
**2.** *Ввод данных о поступившем товаре "с нуля"*<br>
...
**3.** *Проверка кодов маркировки*<br>
...
**2)** актуальная версия Google Chrome;<br>
**3)** стабильный wi-fi;<br>
...
**1)** развернуть приложение на весь экран – нажать кнопку в правом верхнем углу (см. [ниже](#кнопка-на-весь-экран));<br>
...
Для работы с 5S TSD предварительно в программе должна быть настроена интеграция с [API 5S AUTO](../../Администрирование%20и%20настройка%20системы/API%205S%20AUTO/api-5s-auto.md).

**2) Настройка пользователя 5S Cloud**
Для доступа в приложение потребуется ввести логин и пароль пользователя [5S Cloud](../../Администрирование%20и%20настройка%20системы/Создание%20и%20редактирование%20пользователей%205S%20Cloud/sozdanie-i-redaktirovanie-polzovateley-5s-cloud.md). Рекомендуется для каждого устройства создать отдельного пользователя в справочнике Пользователей 5S AUTO и зарегистрировать его в 5S Cloud.
...
См. в статье: [Подключение сканера штрихкодов](../../Торговое%20оборудование/Подключение%20сканера%20штрихкодов/podklyuchenie-skanera-shtrikhkodov.md).

**4) Интеграция с Честным Знаком**
В случае если компания является участником оборота маркированных товаров, необходима регистрация в системе "Честный знак" и настройка интеграции с данным сервисом в программе. Подробнее см. статьи "[Маркировка товаров](../../Маркировка/Маркировка%20товаров/markirovka-tovarov.md)", "[Регистрация в системе Честный знак](../../Маркировка/Регистрация%20в%20системе%20Честный%20знак/registraciya-v-sisteme-chestnyy-znak.md)", "[Настройка интеграции с Честным знаком](../../Маркировка/Настройка%20интеграции%20с%20Честным%20знаком/nastroyka-integracii-s-chestnym-znakom.md)" и другие статьи в разделе "[Маркировка](../../Маркировка/)".

**5) Настройки доступа у пользователя 1С**
Для пользователя 1С должен быть настроен доступ по подразделениям и по складам (см. описание настроек в статьях "[Настройка видимости складов](../../Администрирование%20и%20настройка%20системы/Настройка%20видимости%20складов/nastroyka-vidimosti-skladov.md#1-в-карточке-пользователя)", "[Доступ к данным по подразделениям (RLS)](../../Администрирование%20и%20настройка%20системы/Доступ%20к%20данным%20по%20подразделениям%20%28RLS%29/dostup-k-dannym-po-podrazdeleniyam-rls.md#настройка-доступа-к-подразделениям)" и "[Права на редактирование документов](../../Администрирование%20и%20настройка%20системы/Права%20на%20редактирование%20документов/prava-na-redaktirovanie-dokumentov.md#7-права-на-редактирование-документов-определенного-типа)").
...
**1)** *Минимальная длина кода сканера* – настройка для игнорирования сканером коротких или ошибочных считываний<br>
**2)** *Мое устройство заменяет упр. символ GS строкой* – удаление невидимых символов, замена символа [GS] на строку<br>
