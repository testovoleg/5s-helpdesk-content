...
- [Введение](#введение)
- [Настройка межфирменных продаж](#настройка-межфирменных-продаж)
- [Обработка межфирменных продаж](#обработка-межфирменных-продаж)
  - [Параметры обработки "Межфирменные продажи"](#параметры-обработки-межфирменные-продажи)
  - [Дополнительные возможности обработки "Межфирменные продажи"](#дополнительные-возможности-обработки-межфирменные-продажи)
...
Перед обработкой межфирменных продаж не должно быть нарушений в последовательности документов. Поэтому для начала рекомендуется произвести *[Восстановление последовательностей](../../Администрирование%20и%20настройка%20системы/Восстановление%20последовательностей%20документов/vosstanovlenie-posledovatelnostey-dokumentov.md)*.

***Внимание!*** При записи документов по межфирменным продажам автоматически устанавливаются локальные значения следующих [прав](../../Пользователи%20и%20права/Установка%20прав%20и%20настроек%20пользователя/ustanovka-prav-i-nastroek-polzovatelya.md):
...
2. Задаем период обработки продаж, организацию *Магазин*, процент наценки при необходимости (описание параметров см. *[Параметры обработки "Межфирменные продажи"](#параметры-обработки-межфирменные-продажи)*) и нажимаем кнопку "Создать продажи":
...
3. На вкладке "Операции" проверяем созданные и скорректированные документы:
   ...
   **1.** В начале периода обработки создается документ "Поступление товаров". Документ содержит весь товар, полученный организаций *Магазин* на склад *Закупщика* в течение периода обработки. Для каждого склада *Закупщика* создается отдельный документ.<br>
   **2.** Для документов "Перемещение товаров в производство" и "Реализация товаров" происходит корректировка - распределение по партиям межфирменных продаж.<br>
   **3.** В конце периода обработки создается:<br>
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**1.** Документ "Перемещение товаров" с каждого склада *Закупщика* на виртуальный склад межфирменных продаж.<br>
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**2.** Документ "Реализация товаров" по *Закупщику* контрагенту *Магазина*.<br>
