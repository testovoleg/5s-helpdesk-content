...
<table><tr><td><b>Время чтения:</b> 6 мин.</td><td><b>Создано:</b> 18.09.2026</td></tr></table>

> *Комментарий: черновик. Состав полей формы настроек подключения требуется уточнить у разработчика.*
...
Выпущенный ключ (токен) необходимо скопировать по ссылке *“Скопировать ключ”* либо позже из карточки ключа в разделе *“JWT-ключи”* и использовать в дальнейшем для настройки интеграции в программе – см. [*ниже*](#3-настройка-подключения).

*Примечание:* В отличие от Т-Банка, указывать внешние IP-адреса для обращения к API в интернет-банке Точки не требуется.
...
> *Комментарий: нужен скрин меню «Методы интеграции → Настройки» обработчика «Точка Банк API Загрузка выписок» (как 05 у Т-Банка).*

Добавить новую запись списка:

> *Комментарий: нужен скрин формы «Добавление новой записи списка» (как 06 у Т-Банка).*

Заполнить параметры:

- *Наименование* – наименование интеграции с API Точка Банка;
- *Описание* – описание аккаунта;
- *Токен* – JWT-ключ, полученный при настройке сервиса на стороне банка – см. *[выше](#настройка-на-стороне-банка).*

> *Комментарий: состав полей формы предположительный — по аналогии с Т-Банком и с формой обработчика «Точка Банк СБП» (там ещё Ид. счета, Ид. приложения, Ид. торг. точки). Проверить в программе: нужны ли для выписок Ид. приложения (Client_ID) и Ид. счета, или счета выбираются в регламентном задании.*

*Примечание:* Для загрузки выписок и для приема оплаты через СБП должны быть созданы *разные записи* настроек подключения – каждая в своей интеграции. Даже если используется один и тот же JWT-ключ, запись, созданная для интеграции “Точка Банк СБП”, для загрузки выписок не подходит.

*Примечание:* Отдельные действия по авторизации, как для Сбербанка и Альфа-Банка, для Точка Банка не требуются: подключение работает по токену.
