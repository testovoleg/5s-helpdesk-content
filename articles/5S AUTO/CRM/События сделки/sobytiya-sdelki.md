...
**1) Комментарий.** Окно для создания внутреннего комментария позволяет зафиксировать последние изменения, заметки и прочее с возможностью упомянуть коллегу — в этом случае он получит [уведомление](https://www.5systems.ru/help/rabota-s-uvedomleniyami), — а также прикрепить файл.
...
**2) Ждать.** Функция используется, если клиент по данной сделке попросил «подумать» один или несколько дней. Можно установить количество дней или ожидание любой активности от клиента: сделка будет переведена в отложенные — подробнее см. статью «[Отложенные сделки](https://www.5systems.ru/help/otlozhennye-sdelki)».
