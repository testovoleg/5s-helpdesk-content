...
Мастер подставляется в документ согласно данным регистра **«Сведения адресации»**: программа берет текущего пользователя (через которого выполнен вход в программу), если за ним закреплена роль **«Мастер-консультант»**. Если у пользователя такой роли нет, поле остается пустым.

## Решение

Требуется назначить роль «Мастер-консультант» для пользователя, который должен подставляться в качестве Мастера. Для этого пользователь с правами администратора должен сделать следующие настройки:
...
Кроме подстановки мастера, роль «Мастер-консультант» влияет на адресацию задач и новых сделок в CRM, поэтому регистр «Сведения адресации» обязателен к заполнению для всех, кто работает с CRM и задачами.
