...
Само открытие карточки — это способ уведомления **«Открывать форму объекта»**.
...
**3. Назначьте должность CRM.**

Уведомления адресуются по должностям CRM из регистра **«Сведения адресации»**. Если у сотрудника должность не назначена, адресовать уведомление некому. Откройте карточку пользователя, перейдите на вкладку **«Сведения адресации»**, добавьте строку с нужной должностью (например, «Мастер-консультант» или «Оператор кол-центра») и запишите.
...
Эта галочка находится там же, на вкладке **«Параметры пользователя»**, рядом с внутренним номером. Она предназначена только для служебного пользователя web-сервисов (робота): для обычного сотрудника ее устанавливать не нужно, иначе логика работы нарушается. Снимите галочку и запишите карточку.
