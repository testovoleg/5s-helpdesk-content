...
Есть ли в программе история изменений документа? Например, нужно проверить, кто поменял исполнителя в заказ-наряде.

## Ответ

Да, посмотреть историю можно. Есть три способа, они отличаются глубиной хранения и тем, насколько подробно видно изменение.
...
В окне истории по каждому событию видно дату и время, имя пользователя, компьютер, с которого работали, и само событие — «Данные. Добавление», «Данные. Изменение», «Данные. Проведение».

Так же можно узнать, кем была изменена настройка состояния заказ-наряда: справочник состояний открывается из того же меню «Действия».
