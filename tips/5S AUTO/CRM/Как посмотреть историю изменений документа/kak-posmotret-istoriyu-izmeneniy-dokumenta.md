...
Как посмотреть в программе историю изменений документа — например, проверить, кто поменял исполнителя в заказ-наряде?

## Ответ

Есть несколько способов. Они отличаются глубиной хранения и тем, насколько подробно видно изменение.
...
Тот же механизм работает для справочников. Например, чтобы узнать, кем была изменена настройка доступности состояния заказ-наряда, откройте справочник «Состояния заказ-нарядов», выделите нужное состояние и вызовите «Действия → История объекта». Этот случай разобран в совете [Что делать, если пропадают состояния заказ-нарядов](../../Автосервис/Что%20делать,%20если%20пропадают%20состояния%20заказ-нарядов/chto-delat-esli-propadayut-sostoyaniya-zakaz-naryadov.md).
...
Подробнее см. статью [Версионирование](../../../../articles/5S%20AUTO/Администрирование%20и%20настройка%20системы/Версионирование/versionirovanie.md).
...
- Как посмотреть историю изменений документа
- Есть ли где-нибудь в 1С история изменений в ЗН? Конкретно интересует возможность проверить кто поменял исполнителя в заказ-наряде

## Теги

история объекта, журнал регистрации, версионирование, кто изменил документ, заказ-наряд, 5S Cloud
