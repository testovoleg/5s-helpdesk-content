...
> *Комментарий: нужен скрин — список регистра сведений «Обмен с Audatex» с заполненными графами.*

## Режимы обмена

Обработка обмена вызывается из заказ-наряда: выберите в списке заказ-нарядов нужный документ и нажмите **«Импорт данных из Audatex»** на командной панели. Откроется окно **«Обмен данными с AudaPad Web»**:
...
> *Комментарий: нужны скрины — кнопка «Импорт данных из Audatex» на командной панели списка заказ-нарядов и само окно «Обмен данными с AudaPad Web» с пятью кнопками.*

## Получение данных из AudaPad Web

Режим предназначен для составления калькуляции в AudaPad Web и загрузки ее в заказ-наряд.

1. Нажмите **«Получение данных из AudaPad Web»** — откроется страница AudaPad Web со списком дел.
2. В области **«Учетная запись»** введите логин и пароль пользователя Audatex — получателя задания АХ — и нажмите **«Авторизация»**.
3. Нажмите **«Далее»** внизу страницы.

> *Комментарий: нужен скрин — страница AudaPad Web со списком дел и областью «Учетная запись» (логин, пароль, кнопка «Авторизация»).*
...
1. Заполните обязательные поля и административную информацию, сохраните дело.
2. В главном меню дела заполните административные разделы — вкладки **«Данные дела»**, **«Идентификация ТС»** и **«Данные ТС»**: описание дела, данные о страховке, собственнике и СТО, подробные сведения об автомобиле и его состоянии. Здесь же доступны поиск по VIN-запросу, AudaHistory и расшифровка комплектации ТС.
3. Задайте стоимость нормо-часа или выберите одно из предустановленных партнерств, укажите условия ремонта и метод окраски — от этого зависит расчет.
4. На вкладке **«Описание повреждений»** запустите **OnePad** и подберите ремонтные воздействия, уточнения комплектации и нестандартные позиции.
5. На вкладке **«Параметры расчета»** задайте параметры и рассчитайте калькуляцию. Результаты можно посмотреть в AudaPad Web в выходных данных расчета.

> *Комментарий: нужны скрины по шагам создания дела — форма нового дела, главное меню дела, вкладки «Данные дела» / «Идентификация ТС» / «Данные ТС», окно стоимости и условий ремонта с выбором метода окраски, окно OnePad с подбором ремонтных воздействий, результат расчета калькуляции.*
...
> *Комментарий: нужны скрины — список дел из AudaPad Web с калькуляциями выбранного дела, форма настройки таблицы соответствий, предварительный просмотр загрузки и заказ-наряд с загруженными работами и запчастями.*
...
Здесь же виден список дел, которые ранее загружались в этот заказ-наряд.

> *Комментарий: нужен скрин — окно режима «Передача данных в AudaPad Web» с назначением дела другому пользователю и прикреплением заказ-наряда к делу.*
